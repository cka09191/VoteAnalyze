...
sub make_instance {
    my ($class, $context) = @_;
    Carp::croak("Usage: VoteAnalyze::Web::View->make_instance(\$context_class)") if @_!=2;

    my $view_conf = $context->config->{'Text::Xslate'} || +{};
    unless (exists $view_conf->{path}) {
        my $tmpl_path = File::Spec->catdir($context->base_dir(), 'tmpl/web');
        if ( -d $tmpl_path ) {
            # tmpl/web
            $view_conf->{path} = [ $tmpl_path ];
        } else {
            my $share_tmpl_path = eval { File::Spec->catdir(File::ShareDir::dist_dir('VoteAnalyze'), 'tmpl/web') };
            if ($share_tmpl_path) {
